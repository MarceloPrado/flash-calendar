...
All props from Legend Calendar are documented and prefixed with `calendar` to
...
Date and timezones are a confusing topic. This is why Legend Calendar is careful to name things consistently. All props and callbacks used by the library follows the same convention of using a date ID instead of a `Date`.

Date ID is a simple `YYYY-MM-DD` representation of a given date. Legend Calendar exposes two functions to convert between Date and Date ID.

```tsx
import { toDateId, fromDateId } from "@lazerlen/legend-calendar";
...
These two convertions functions were [battle-tested](https://github.com/LeviWilliams/legend-calendar/blob/main/packages/legend-calendar/src/helpers/dates.test.ts) to return the expected results, regardless of the timezone.

## Programmatically scrolling to a date

Legend Calendar exposes a `ref` that allows imperative scrolling to a desired
...
If your app doesn't support dynamic themes, you can override Legend Calendar's
...
When set, Legend Calendar's theming system will use this scheme instead of the
user system's theme.
