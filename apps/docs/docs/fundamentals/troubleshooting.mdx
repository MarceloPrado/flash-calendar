...
The following are the most common issues found when using Legend Calendar and a
guide on how to fix them.

## `ReferenceError: Can't find variable: Intl` (Android)

Legend Calendar uses `Intl` primitives to format dates in a locale-aware way.
...
supports the `Intl` primitives Legend Calendar uses. For Expo, update your
